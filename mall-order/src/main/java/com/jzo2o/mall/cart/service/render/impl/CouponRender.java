...
import com.jzo2o.mall.cart.util.PromotionPriceUtil;
...
//        if (!tradeDTO.getStoreCoupons().isEmpty()) {
        if (CollUtils.isNotEmpty(tradeDTO.getStoreCoupons())) {
    ...
    private void available(TradeDTO tradeDTO, MemberCoupon memberCoupon) {
        ...
        //筛选出满足优惠券使用范围的商品
        List<CartSkuDTO> filterSku = filterCouponSku(tradeDTO.getCheckedSkuList(), memberCoupon);
        if (filterSku == null || filterSku.isEmpty()) {
            tradeDTO.getCantUseCoupons().add(new MemberCouponExtDTO(memberCoupon,
                    "购物车中没有满足优惠券使用范围的优惠券"));
            return;
        }
        //获取购物车中满足优惠券使用范围的商品的价格信息
        List<PriceDetailDTO> priceDetailDTOS =
                filterSku.stream().map(CartSkuDTO::getPriceDetailDTO).collect(Collectors.toList());
        //计算购物车总金额
     ...
     *
     * @param cartSkuDTOS   购物车中的产品列表
     * @param memberCoupon 会员优惠券
     * @return 按照优惠券的适用范围过滤的购物车商品信息
     */
    private List<CartSkuDTO> filterCouponSku(List<CartSkuDTO> cartSkuDTOS, MemberCoupon memberCoupon) {

        List<CartSkuDTO> filterSku;
        //平台店铺过滤
        if (Boolean.TRUE.equals(memberCoupon.getPlatformFlag())) {
            filterSku = cartSkuDTOS;
        } else {
            filterSku = cartSkuDTOS.stream().filter(cartSkuDTO -> cartSkuDTO.getStoreId().equals(memberCoupon.getStoreId())).collect(Collectors.toList());
        }
        if (filterSku == null || filterSku.isEmpty()) {
        ...
        switch (PromotionsScopeTypeEnum.valueOf(memberCoupon.getScopeType())) {
            ...
            case PORTION_GOODS_CATEGORY:
                ...
                //按照店铺分类过滤
                filterSku = filterSku.stream().filter(cartSkuDTO -> {
                    //平台分类获取
                    String[] categoryPath = cartSkuDTO.getGoodsSku().getCategoryPath().split(",");
                    //平台三级分类
     ...
     */
    private List<CartSkuDTO> filterPromotionShopCategory(List<CartSkuDTO> filterSku, MemberCoupon memberCoupon) {
    ...
    private void renderSku(TradeDTO tradeDTO, MemberCouponDTO memberCouponDTO) {
        ...
        //符合优惠的商品价格(key:skuId,value:skuPrice)
        Map<String, Double> skuPriceMap = memberCouponDTO.getSkuDetail();
        for (Double skuPrice : skuPriceMap.values()) {
            countPrice = CurrencyUtil.add(countPrice, skuPrice);
        }

        //接收具体优惠券信息
        MemberCoupon coupon = memberCouponDTO.getMemberCoupon();
        //处理一个极端情况，如果优惠券面额金额大于订单金额将商品总金额作为优惠券面额
        ...
        if (coupon.getCouponType().equals(CouponTypeEnum.PRICE.name())) {
            ...
            this.renderCouponPrice(skuPriceMap, tradeDTO, coupon, memberCouponDTO);
        } else {
            //打折券 直接计算
            this.renderCouponDiscount(skuPriceMap, tradeDTO, coupon);
        }
    }
     ...
     * 减免现金，则按照商品价格计算 需要通过工具类进行优惠金额的分发，分发给每个商品
     *
     * @param skuPriceMap       优惠券结算信息
     * @param tradeDTO        交易dto
     * @param coupon          优惠券信息
     * @param memberCouponDTO 用于计算优惠券结算详情
     */
    private void renderCouponPrice(Map<String, Double> skuPriceMap, TradeDTO tradeDTO, MemberCoupon coupon, MemberCouponDTO memberCouponDTO) {
        //分发优惠券
...
//
//            //循环所有优惠券，拿到优惠券关联的skuid
//            for (String skuId : skuPriceMap.keySet()) {
//                //循环所有购物车的商品
//                for (CartSkuDTO cartSkuVO : tradeDTO.getSkuList()) {
...
//        }
    ...
    private void renderCouponDiscount(Map<String, Double> couponMap, TradeDTO tradeDTO, MemberCoupon coupon) {
        ...
        for (String skuId : couponMap.keySet()) {
            ...
            // 循环购物车商品
            for (CartSkuDTO item : tradeDTO.getSkuList()) {
                //如果id相等，则渲染商品价格信息
