package com.jzo2o.mall.cart.util;
...
public class PromotionPriceUtil {
    ...
    public static void recountPrice(TradeDTO tradeDTO, Map<String, Double> skuPromotionDetail, Double discountPrice,
        ...
        //极端情况，如果优惠金额大于商品总金额，则优惠金额=商品总金额，不能成为负数
                ...
                for (CartSkuDTO cartSkuDTO : tradeDTO.getSkuList()) {
                    ...
                    if (cartSkuDTO.getGoodsSku().getId().equals(skuId)) {
                        ...
                        if (promotionTypeEnum == PromotionTypeEnum.COUPON) {
                            cartSkuDTO.getPriceDetailDTO().setCouponPrice(cartSkuDTO.getPriceDetailDTO().getGoodsPrice());
                        } else {
                            cartSkuDTO.getPriceDetailDTO().setDiscountPrice(cartSkuDTO.getPriceDetailDTO().getGoodsPrice());
                        }
                    }
                }
            }
        ...
        //获取购物车信息
        List<CartSkuDTO> skuDTOList = tradeDTO.getSkuList();
        ...
        for (String skuId : skuPromotionDetail.keySet()) {
            ...
            for (CartSkuDTO cartSkuDTO : skuDTOList) {
                ...
                if (cartSkuDTO.getGoodsSku().getId().equals(skuId)) {
                    ...
                    if (count > 0) {
                        //商品金额占比
                        double point = CurrencyUtil.div(cartSkuDTO.getPriceDetailDTO().getGoodsPrice(), totalPrice, 4);
                        //商品优惠金额
                    ...
                    calculateCartSkuPromotionsPrice(cartSkuDTO, skuDiscountPrice, promotionTypeEnum, activityId);
                }
            }
        }
        System.out.println("");

//        calculateNotEnoughPromotionsPrice(skuDTOList, skuPromotionDetail, discountPrice, totalPrice, promotionTypeEnum, activityId);
     ...
     * 计算购物车商品优惠金额
     *
     * @param cartSkuDTO         购物车商品
     * @param skuDiscountPrice  商品优惠金额
     * @param promotionTypeEnum 优惠类型
     * @param activityId        优惠活动id
     */
    private static void calculateCartSkuPromotionsPrice(CartSkuDTO cartSkuDTO, Double skuDiscountPrice, PromotionTypeEnum promotionTypeEnum,
                                                        String activityId) {
        //优惠券金额，则计入优惠券 ，其他则计入总的discount price
        if (promotionTypeEnum == PromotionTypeEnum.COUPON) {

            cartSkuDTO.getPriceDetailDTO().setCouponPrice(
                    CurrencyUtil.add(cartSkuDTO.getPriceDetailDTO().getCouponPrice(), skuDiscountPrice));

            cartSkuDTO.getPriceDetailDTO().addDiscountPriceItem(
            ...
            cartSkuDTO.getPriceDetailDTO().setSiteCouponPrice(
                    CurrencyUtil.add(cartSkuDTO.getPriceDetailDTO().getCouponPrice(), skuDiscountPrice));

            cartSkuDTO.getPriceDetailDTO().setCouponPrice(
                    CurrencyUtil.add(cartSkuDTO.getPriceDetailDTO().getCouponPrice(), cartSkuDTO.getPriceDetailDTO().getSiteCouponPrice()));
...
//        else {
//            cartSkuDTO.getPriceDetailDTO().setDiscountPrice(
//                    CurrencyUtil.add(cartSkuDTO.getPriceDetailDTO().getDiscountPrice(), skuDiscountPrice));
//
...
//        }
     ...
     * @param activityId         优惠活动id
     */
    private static void calculateNotEnoughPromotionsPrice(List<CartSkuDTO> skuVOList, Map<String, Double> skuPromotionDetail, Double discountPrice,
        ...
        // 获取剩余金额不足优惠金额的商品
        List<CartSkuDTO> unEnoughSku = skuVOList.stream().filter(k -> {
        ...
        if (!unEnoughSku.isEmpty()) {
            ...
            for (CartSkuDTO cartSkuDTO : skuVOList) {
                if (unEnoughSku.isEmpty()) {
                    break;
                }
                if (skuPromotionDetail.containsKey(cartSkuDTO.getGoodsSku().getId()) && unEnoughSku.stream().noneMatch(k -> k.getGoodsSku().getId().equals(cartSkuDTO.getGoodsSku().getId()))) {
                    // 商品金额占比
                    double point = CurrencyUtil.div(cartSkuDTO.getPriceDetailDTO().getGoodsPrice(), totalPrice, 4);
                    // 商品优惠金额
                    Double skuDiscountPrice = CurrencyUtil.mul(discountPrice, point);
                    ...
                    calculateCartSkuPromotionsPrice(cartSkuDTO, sub, promotionTypeEnum, activityId);
                    // 从不足商品列表中移除
