...
import com.jzo2o.mall.aftersale.mapper.AfterSaleMapper;
...
import com.jzo2o.mall.aftersale.model.enums.AfterSaleRefundWayEnum;
import com.jzo2o.mall.aftersale.model.enums.AfterSaleStatusEnum;
...
//    private RocketMQTemplate rocketMQTemplate;
    ...
    @Override
    public IPage<AfterSaleDTO> getAfterSalePages(AfterSaleSearchParamsDTO saleSearchParams) {
        IPage<AfterSaleDTO> afterSaleDTOIPage = baseMapper.queryByParams(PageUtils.initPage(saleSearchParams), saleSearchParams.queryWrapper());
        return afterSaleDTOIPage;
    }

    @Override
    public List<AfterSale> exportAfterSaleOrder(AfterSaleSearchParamsDTO saleSearchParams) {
        return this.list(saleSearchParams.queryWrapper());
    }

    @Override
    public AfterSaleDTO getAfterSale(String sn) {
        return this.baseMapper.getAfterSaleVO(sn);
    }

    @Override
    public AfterSaleApplyDTO getAfterSaleVO(String sn) {

        AfterSaleApplyDTO afterSaleApplyVO = new AfterSaleApplyDTO();
...
//        this.sendAfterSaleMessage(afterSale);
    ...
    @Override
    public TracesDTO deliveryTraces(String afterSaleSn) {
...
//        this.sendAfterSaleMessage(afterSale);
    ...
    private void refund(AfterSale afterSale) {
...
//        this.sendAfterSaleMessage(afterSale);
    ...
    public StoreAfterSaleAddressDTO getStoreAfterSaleAddressDTO(String sn) {
        AfterSale bySn = this.getBySn(sn);
        String storeId = bySn.getStoreId();
        return storeDetailService.getStoreAfterSaleAddressDTO(storeId);
    }

    @Override
    public void completeAfterSale(String sn) {
        //查询售后单
        AfterSale afterSale = this.getBySn(sn);
        if(afterSale!=null){
            afterSale.setServiceStatus(AfterSaleStatusEnum.COMPLETE.name());
            this.updateAfterSale(sn, afterSale);
        }
    ...
    private AfterSale addAfterSale(AfterSaleDTO afterSaleDTO) {
        ...
        //写入会员信息
        afterSale.setMemberId(tokenUser.getIdString());
