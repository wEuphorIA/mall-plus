...
public interface OrderItemMapper extends BaseMapper<OrderItem> {
     ...
     * @return 订单子项列表
     */
    @Select("SELECT * FROM oms_order_item AS oi INNER JOIN oms_order AS o ON oi.order_sn=o.sn ${ew.customSqlSegment}")
    List<OrderItem> waitOperationOrderItem(@Param(Constants.WRAPPER) Wrapper<OrderSimpleDTO> queryWrapper);
     ...
     */
    @Update(" UPDATE oms_order_item AS oi " +
            "INNER JOIN oms_order AS o ON oi.order_sn = o.sn " +
            "INNER JOIN oms_after_sale AS af ON af.order_item_sn = oi.sn " +
