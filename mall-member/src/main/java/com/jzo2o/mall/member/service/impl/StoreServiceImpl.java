...
import com.jzo2o.mall.member.mapper.StoreMapper;
...
//    private FootprintService footprintService;
    ...
    @Override
    public IPage<StoreDTO> findByConditionPage(StoreSearchParams storeSearchParams) {
        return this.baseMapper.getStoreList(PageUtils.initPage(storeSearchParams), storeSearchParams.queryWrapper());
    }

    @Override
    public StoreDTO getStoreDetail() {
        AuthUser currentUser =  UserContext.getCurrentUser();
        StoreDTO storeVO = this.baseMapper.getStoreDetail(currentUser.getStoreId());
...
//            rocketMQTemplate.asyncSend(destination, store, RocketmqSendCallbackBuilder.commonCallback());
    ...
    public boolean audit(String id, Integer passed) {
        ...
        if (passed == 0) {
            ...
            clerkAddDTO.setUsername(member.getUsername());
