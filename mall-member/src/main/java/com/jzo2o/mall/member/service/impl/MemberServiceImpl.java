...
import com.jzo2o.mall.common.utils.CookieUtil;
...
//                MemberTagsEnum.MEMBER_REGISTER.name(), member));
    ...
    public Member editOwn(MemberEditDTO memberEditDTO) {
        AuthUser tokenUser =  UserContext.getCurrentUser();
        Objects.requireNonNull(tokenUser);
        //查询会员信息
...
//        rocketMQTemplate.asyncSend(destination, member, RocketmqSendCallbackBuilder.commonCallback());
    ...
    public Member modifyPass(String oldPassword, String newPassword) {
        ...
        Member member = this.getById(tokenUser.getIdString());
        //判断旧密码输入是否正确
...
//            connectService.deleteByMemberId(member.getId());
    ...
    @Override
    @Transactional
    public Member register(String userName, String password, String mobilePhone) {
        //检测会员信息
...
//        return memberTokenGenerate.createToken(member, false);
    ...
    public boolean changeMobile(String mobile) {
        ...
        if (!Objects.equals(tokenUser.getIdString(), member.getId())) {
            throw new ServiceException(ResultCode.USER_NOT_LOGIN);
        }
        //修改会员手机号
        LambdaUpdateWrapper<Member> lambdaUpdateWrapper = Wrappers.lambdaUpdate();
...
//        }
    ...
    @Override
    public IPage<MemberDTO> getMemberPage(MemberSearchDTO memberSearchVO, PageVO page) {
        ...
        return this.baseMapper.pageByMemberVO(PageUtils.initPage(page), queryWrapper);
    }
...
//    }
    ...
    @Override
    public long getMemberNum(MemberSearchDTO memberSearchDTO) {
        QueryWrapper<Member> queryWrapper = Wrappers.query();
        //用户名查询
        queryWrapper.like(CharSequenceUtil.isNotBlank(memberSearchDTO.getUsername()), "username", memberSearchDTO.getUsername());
        //按照电话号码查询
        queryWrapper.like(CharSequenceUtil.isNotBlank(memberSearchDTO.getMobile()), "mobile", memberSearchDTO.getMobile());
        //按照状态查询
        queryWrapper.eq(CharSequenceUtil.isNotBlank(memberSearchDTO.getDisabled()), "disabled",
                memberSearchDTO.getDisabled().equals(SwitchEnum.OPEN.name()) ? 1 : 0);
        queryWrapper.orderByDesc("create_time");
...
//    }
    ...
    public MemberDTO getMember(String id) {
        return new MemberDTO(this.getById(id));
    }
