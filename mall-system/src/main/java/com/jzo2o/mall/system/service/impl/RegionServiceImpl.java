...
import com.jzo2o.mall.common.utils.HttpClientUtils;
import com.jzo2o.mall.common.utils.SnowFlake;
import com.jzo2o.mall.system.mapper.RegionMapper;
...
public class RegionServiceImpl extends ServiceImpl<RegionMapper, Region> implements RegionService {
    ...
    private List<RegionDTO> regionTree(List<Region> regions) {
        List<RegionDTO> regionVOS = new ArrayList<>();
        regions.stream().filter(region -> ("province").equals(region.getLevel())).forEach(item -> regionVOS.add(new RegionDTO(item)));
        regions.stream().filter(region -> ("city").equals(region.getLevel())).forEach(item -> {
            for (RegionDTO region : regionVOS) {
                if (region.getId().equals(item.getParentId())) {
                    region.getChildren().add(new RegionDTO(item));
                }
            }
        });
        return regionVOS;
