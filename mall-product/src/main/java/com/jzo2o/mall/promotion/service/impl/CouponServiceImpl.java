...
import com.jzo2o.mall.promotion.mapper.CouponMapper;
...
//        this.couponActivityItemService.removeByCouponId(ids);
     ...
     */
    @Override
    public IPage<CouponDTO> pageVOFindAll(CouponSearchParams searchParams, PageVO page) {
        IPage<Coupon> couponIPage = super.pageFindAll(searchParams, page);
        List<CouponDTO> couponVOList = couponIPage.getRecords().stream().map(CouponDTO::new).collect(Collectors.toList());
        return PageUtils.convertPage(couponIPage, couponVOList);
    }
     ...
     */
    @Override
    public CouponDTO getDetail(String couponId) {
        CouponDTO couponVO = new CouponDTO(this.getById(couponId));
...
//            this.couponActivityItemService.removeByCouponId(ids);
    ...
    public void checkPromotions(Coupon coupon) {
        ...
        this.checkCouponScope((CouponDTO) coupon);
    }
...
//        }
    ...
    public boolean updatePromotionsGoods(Coupon promotions) {
        ...
        if (!PromotionsStatusEnum.CLOSE.name().equals(promotions.getPromotionStatus()) &&
                ...
                promotions instanceof CouponDTO) {
            CouponDTO couponDTO = (CouponDTO) promotions;
            this.promotionGoodsService.deletePromotionGoods(Collections.singletonList(promotions.getId()));
            List<PromotionGoods> promotionGoodsList = PromotionTools.promotionGoodsInit(couponDTO.getPromotionGoodsList(), couponDTO, this.getPromotionType());
...
//            applicationEventPublisher.publishEvent(new TransactionCommitSendMQEvent("删除商品促销事件", rocketmqCustomProperties.getGoodsTopic(), GoodsTagsEnum.DELETE_GOODS_INDEX_PROMOTIONS.name(), JSONUtil.toJsonStr(build)));
     ...
     */
    private void checkCouponScope(CouponDTO coupon) {
     ...
     * @param coupon 优惠券信息
     */
    private void checkCouponPortionGoods(CouponDTO coupon) {
