package com.jzo2o.mall.promotion.service.impl;
...
import com.jzo2o.mall.promotion.mapper.SeckillApplyMapper;
import com.jzo2o.mall.promotion.model.domain.BasePromotions;
...
import com.jzo2o.mall.promotion.model.dto.*;
import com.jzo2o.mall.promotion.model.enums.PromotionsApplyStatusEnum;
...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    @Override
    public List<SeckillTimelineDTO> getSeckillTimeline(String date) {
        List<SeckillTimelineDTO> seckillTimelineDTOS = new ArrayList<>();
        try {
            //获取当天秒杀时间轴
            List<SeckillTimelineDTO> seckillTimelineInfo = getSeckillTimelineInfoAll(date);

            Calendar c = Calendar.getInstance();
            //当前小时
            int hour = c.get(Calendar.HOUR_OF_DAY);
            //从seckillTimelineInfo中提取小时，最终得到一个int数组
            Integer[] hoursSored = seckillTimelineInfo.stream().map(SeckillTimelineDTO::getTimeLine).map(Integer::valueOf).toArray(Integer[]::new);
            ...
            for (int i = 0; i < hoursSored.length; i++) {
                ...
                if (hoursSoredHour || lastHour) {
                    SeckillTimelineDTO tempTimeline = seckillTimelineInfo.get(i);
                    seckillTimelineDTOS.add(tempTimeline);
                }
            }
            return seckillTimelineDTOS;
        } catch (Exception e) {
    ...
    @Override
    public List<SeckillGoodsDTO> getSeckillGoods(Integer timeline) {
        try {
            List<SeckillGoodsDTO> seckillGoodsVoS = new ArrayList<>();
            String now = DateUtils.format(LocalDateTime.now(), "yyyy-MM-dd");
            //获取
            List<SeckillTimelineDTO> seckillTimelineToCache = getSeckillTimelineInfoAll(now);
            Optional<SeckillTimelineDTO> first = seckillTimelineToCache.stream().filter(i -> i.getTimeLine().equals(timeline)).findFirst();
            if (first.isPresent()) {
    ...
    public IPage<SeckillApply> getSeckillApplyPage(SeckillSearchParams queryParam, PageVO pageVo) {
        IPage<SeckillApply> seckillApplyPage = this.page(PageUtils.initPage(pageVo), queryParam.queryWrapper());
    ...
    @Override
    @CacheEvict(value = RedisConstants.PromotionCacheName.SECKILL_CACHE, key = "'ACTIVE_SECKILL'")
    @Transactional(rollbackFor = Exception.class)
    public void addSeckillApply(String seckillId, String storeId, List<SeckillApplyDTO> seckillApplyList) {
        ...
        for (SeckillApplyDTO seckillApply : seckillApplyList) {
            //获取参与活动的商品信息
        ...
        if (!promotionGoodsList.isEmpty()) {
            ...
            //将秒杀商品库存写入redis
            promotionGoodsService.updatePromotionGoodsStock(promotionGoodsList);
        }
        //设置秒杀活动的商品数量、店铺数量
...
//        this.seckillService.deleteEsGoodsSeckill(seckill, Collections.singletonList(seckillApply.getSkuId()));
     ...
     */
    private void checkSeckillApplyList(String hours, List<SeckillApplyDTO> seckillApplyList) {
     ...
     * @return 秒杀活动信息
     */
    private List<SeckillTimelineDTO> getSeckillTimelineInfo(String date) {
        List<SeckillTimelineDTO> timelineList = new ArrayList<>();
        LambdaQueryWrapper<Seckill> queryWrapper = new LambdaQueryWrapper<>();
        //查询当天时间段内的秒杀活动活动
//        Date now = new Date();
        //将yyyy-MM-dd转成date
        LocalDateTime localDateTime = DateUtils.parse(date, "yyyy-MM-dd");
        ...
        queryWrapper.le(BasePromotions::getEndTime, DateUtil.endOfDay(now));
        //查询当天的秒杀活动
...
//        for (Seckill seckill : seckillList) {
            ...
            for (int i = 0; i < hoursSored.length; i++) {
                SeckillTimelineDTO tempTimeline = new SeckillTimelineDTO();
                boolean hoursSoredHour = (hoursSored[i] >= hour || ((i + 1) < hoursSored.length && hoursSored[i + 1] > hour));
                boolean lastHour = i == hoursSored.length - 1 && hoursSored[i] < hour;
                if (hoursSoredHour || lastHour) {
...
//                    //秒杀活动的时刻
                    long timeLine = com.jzo2o.mall.common.utils.DateUtil.getDateline(date + " " + hoursSored[i], "yyyy-MM-dd HH");
//                    //距离活动开始时间
...
//        }
     ...
     * @return 秒杀活动信息
     */
    @Cacheable(value = RedisConstants.PromotionCacheName.SECKILL_CACHE, key = "#date",unless = "#result == null || #result.isEmpty()",cacheManager = RedisConstants.CacheManager.FOREVER)
    public List<SeckillTimelineDTO> getSeckillTimelineInfoAll(String date) {
        List<SeckillTimelineDTO> timelineList = new ArrayList<>();
        LambdaQueryWrapper<Seckill> queryWrapper = new LambdaQueryWrapper<>();
        //查询当天时间段内的秒杀活动活动
//        Date now = new Date();
...
//    private List<SeckillTimelineDTO> getSeckillTimelineInfo() {
//        List<SeckillTimelineDTO> timelineList = new ArrayList<>();
//        LambdaQueryWrapper<Seckill> queryWrapper = new LambdaQueryWrapper<>();
//        //查询当天时间段内的秒杀活动活动
//        Date now = new Date();
...
//    }
     ...
     * @return 当时间秒杀活动的商品数据
     */
    private List<SeckillGoodsDTO> wrapperSeckillGoods(Integer startTimeline, String seckillId) {
        List<SeckillGoodsDTO> seckillGoodsVoS = new ArrayList<>();
        ...
        if (!seckillApplyList.isEmpty()) {
            ...
            for (SeckillApply seckillApply : collect) {
                ...
                promotionGoodsSearchParams.setSkuId(seckillApply.getSkuId());
                promotionGoodsSearchParams.setPromotionId(seckillApply.getSeckillId());
                //秒杀商品信息
                PromotionGoods promotionsGoods = promotionGoodsService.getPromotionsGoods(promotionGoodsSearchParams);
                if (goodsSku != null) {
                    SeckillGoodsDTO goodsDTO = new SeckillGoodsDTO();
                    BeanUtil.copyProperties(seckillApply, goodsDTO);
                    goodsDTO.setGoodsImage(goodsSku.getThumbnail());
                    goodsDTO.setGoodsId(goodsSku.getGoodsId());
                    goodsDTO.setGoodsName(goodsSku.getGoodsName());
                    //秒杀商品信息
                    goodsDTO.setPromotionGoods(promotionsGoods);
                    seckillGoodsVoS.add(goodsDTO);
     ...
     */
    private void checkSeckillGoodsSku(Seckill seckill, SeckillApplyDTO seckillApply, GoodsSku goodsSku, DateTime startTime) {
        //活动库存不能大于商品库存
