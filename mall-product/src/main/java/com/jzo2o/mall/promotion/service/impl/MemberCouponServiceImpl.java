...
import com.jzo2o.mall.promotion.mapper.MemberCouponMapper;
...
import com.jzo2o.mall.promotion.model.enums.*;
...
public class MemberCouponServiceImpl extends ServiceImpl<MemberCouponMapper, MemberCoupon> implements MemberCouponService {
    ...
    public IPage<MemberCoupon> getMemberCoupons(MemberCouponSearchParams param, PageVO pageVo) {
        ...
        Page<MemberCoupon> page = this.page(PageUtils.initPage(pageVo), queryWrapper);
        if (page.getRecords() != null && page.getRecords().size() > 0) {
            if (page.getRecords().stream().anyMatch(i -> i.getEndTime().before(new Date()))) {
                ...
                return this.page(PageUtils.initPage(pageVo), queryWrapper);
            }
        }
        return page;
    }
...
//    @Cacheable(key = "#memberId")
    public List<MemberCoupon> getMemberCoupons(String memberId) {
        ...
        searchParams.setMemberId(Objects.requireNonNull(authUser).getIdString());
    ...
    public void used(String memberId, String memberCouponId){
        MemberCoupon memberCoupon = getById(memberCouponId);

        if (memberCoupon == null) {
            throw new ServiceException(ResultCode.COUPON_EXPIRED);
        }
        //循环处理
            if (!memberCoupon.getMemberCouponStatus().equals(MemberCouponStatusEnum.NEW.name())) {
                throw new ServiceException(ResultCode.COUPON_EXPIRED);
            }
        memberCoupon.setMemberCouponStatus(MemberCouponStatusEnum.USED.name());
        memberCoupon.setConsumptionTime(new Date());
...
//        boolean update = this.update(updateWrapper);
        //更新优惠券表,使用次数减一
        for (String memberCouponId : memberCouponIds) {
            MemberCoupon memberCoupon = this.getById(memberCouponId);
            //如果是平台优惠券不能恢复
            if (memberCoupon.getPlatformFlag()) {
               continue;
            }
            //更新MemberCoupon表
            LambdaUpdateWrapper<MemberCoupon> updateWrapper = new LambdaUpdateWrapper<>();
            updateWrapper.eq(MemberCoupon::getId, memberCouponId);
            updateWrapper.ne(MemberCoupon::getMemberCouponStatus, MemberCouponStatusEnum.NEW.name());
            updateWrapper.set(MemberCoupon::getMemberCouponStatus, MemberCouponStatusEnum.NEW.name());
            updateWrapper.set(MemberCoupon::getConsumptionTime, null);
            boolean update = this.update(updateWrapper);
            if(update){
                //更新coupon表
                String couponId = getById(memberCouponId).getCouponId();
                couponService.usedCoupon(couponId, -1);
            }
        }
        return true;
    }
    ...
    @Override
    public Page<MemberCouponExtDTO> getMemberCouponsPage(Page<MemberCoupon> page, MemberCouponSearchParams param) {
        QueryWrapper<MemberCouponExtDTO> queryWrapper = new QueryWrapper<>();
