...
import com.jzo2o.mall.promotion.mapper.PromotionGoodsMapper;
...
//    private SeckillApplyService seckillApplyService;
    ...
    public Page<PromotionGoods> pageFindAll(PromotionGoodsSearchParams searchParams, PageVO pageVo) {
        return this.page(PageUtils.initPage(pageVo), searchParams.queryWrapper());
    }
     ...
     */
    @Override
    public List<PromotionGoods> getValidPromotionsGoods(String skuId, List<String> promotionTypes) {
        QueryWrapper<PromotionGoods> queryWrapper = new QueryWrapper<>();
...
////        if (goodsIndex == null) {
////            GoodsDTO goodsDTO = this.goodsService.getGoodsVO(dataSku.getGoodsId());
////            //重建索引
////            goodsIndex = goodsIndexService.getResetEsGoodsIndex(dataSku, goodsDTO.getGoodsParamsDTOList());
////        }
...
//                Optional<Map.Entry<String, Object>> containsPromotion = goodsIndex.getPromotionMap().entrySet().stream().filter(i -> i.getKey().contains(PromotionTypeEnum.SECKILL.name()) ).findFirst();
