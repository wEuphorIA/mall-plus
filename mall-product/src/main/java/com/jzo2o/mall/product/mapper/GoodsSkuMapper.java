...
public interface GoodsSkuMapper extends BaseMapper<GoodsSku> {
     ...
     * @return 全部skuId的集合
     */
    @Select("SELECT id FROM pms_goods_sku WHERE goods_id = #{goodsId}")
    List<String> getGoodsSkuIdByGoodsId(String goodsId);


    @Insert("replace into pms_goods_sku (\n" +
            "\tid,\n" +
            "\tgoods_id,\n" +
            "\tspecs,\n" +
            "\tsimple_specs,\n" +
     ...
     * @return 售后VO分页
     */
    @Select("SELECT *,g.params as params FROM pms_goods_sku gs inner join pms_goods g on gs.goods_id = g.id ${ew.customSqlSegment}")
    IPage<GoodsSkuDTO> queryByParams(IPage<GoodsSkuDTO> page, @Param(Constants.WRAPPER) Wrapper<GoodsSkuDTO> queryWrapper);

    @Select("SELECT id as sku_id, quantity, goods_id FROM pms_goods_sku ${ew.customSqlSegment}")
